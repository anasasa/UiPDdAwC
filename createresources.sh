...
{
	az deployment group create --name "hdi${myindex}" --resource-group "group${myindex}" --template-file ${DIR}/HDI/hdi_template.json --parameters ${DIR}/HDI/hdi_parameters.json -p clusterName="hdi${myindex}" -p clusterLoginPassword="${hdi_password}" -p sshPassword="${hdi_password}" -p storageName="hdi${myindex}storage"
